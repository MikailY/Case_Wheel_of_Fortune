...
namespace WheelOfFortune
{
    ...
    public class WheelOfFortuneDialog : MonoBehaviour
    {
        ...
        public void Init(WheelOfFortuneDialogModel model)
        {
            ...
            spinContainerComponent.Set(firstStage);
            superZoneCounterComponent.Set(GetNextZoneIndexByType(model, firstStage, 2));
            safeZoneCounterComponent.Set(GetNextZoneIndexByType(model, firstStage, 1));
        ...
        private void SpinContainerComponentOnOnSpinCompleted()
        {
            ...
            void OnRewardAnimationComplete(RewardModel reward)
            {
                ...
                stagesContainerComponent.MoveToNextStage(nextStage.Index,
                    () => OnNextStageAnimationFinished(nextStage));
            }

            void OnNextStageAnimationFinished(StageModel nextStage)
            {
                spinContainerComponent.Set(nextStage);
                superZoneCounterComponent.Set(GetNextZoneIndexByType(_model, _currentStage, 2));
                safeZoneCounterComponent.Set(GetNextZoneIndexByType(_model, _currentStage, 1));
                spinContainerComponent.ShowSpinButton();
                rewardsContainerComponent.ShowExitButton();
        ...
        private static int GetNextZoneIndexByType(WheelOfFortuneDialogModel model, StageModel currentStage, int type)
        {
            var nextZone =
                model.Stages.FirstOrDefault(x => x.Type == type && x.Index > currentStage.Index);
